...
async function prijava() {
    try {
        ...
        const odgovor = await axios.post('http://localhost:3000/user/login', {
            ime: ime.value,
            password: password.value
        })

        sessionStorage.setItem('jwt', odgovor.data.jwt_token)

        axios.defaults.headers.common['Authorization'] =`Bearer ${odgovor.data.jwt_token}`

        console.log('Prijava uspješna:', odgovor.data);
...
<template>
    <div
        class="min-h-screen flex items-center justify-center text-white, bg-orange-600"
        :style="{
    ...
    >
        <div class="p-6 max-w-md w-full">
            ...
            <div class="bg-slate-800/90 p-6 rounded-lg shadow-lg">
                ...
                <form @submit.prevent="prijava" class="flex flex-col gap-4">
